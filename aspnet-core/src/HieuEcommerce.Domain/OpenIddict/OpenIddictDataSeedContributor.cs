...
namespace HieuEcommerce.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("HieuEcommerce") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "HieuEcommerce",
                DisplayName = "HieuEcommerce API",
                Resources = { "HieuEcommerce" }
            });
        }
        if (await _openIddictScopeRepository.FindByNameAsync("HieuEcommerce.Admin") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor
            {
                Name = "HieuEcommerce.Admin",
                DisplayName = "HieuEcommerce API Admin",
                Resources = { "HieuEcommerce.Admin" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        adminScopes.Add("HieuEcommerce.Admin");

        var clientScopes = new List<string>();
        clientScopes.AddRange(commonScopes);
        clientScopes.Add("HieuEcommerce");

        var configurationSection = _configuration.GetSection("OpenIddict:Applications");

        //Admin Client
        var webAdminClientId = configurationSection["HieuEcommerce_Admin:ClientId"];
        if (!webAdminClientId.IsNullOrWhiteSpace())
        {
            var adminWebClientRootUrl = configurationSection["HieuEcommerce_Admin:RootUrl"].TrimEnd('/'); 
            await CreateApplicationAsync(
                name: webAdminClientId,
                type: OpenIddictConstants.ClientTypes.Confidential,
                consentType: OpenIddictConstants.ConsentTypes.Implicit,
                displayName: "Admin Application",
                secret: configurationSection["HieuEcommerce_Admin:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["HieuEcommerce_Admin_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["HieuEcommerce_Admin_Swagger:RootUrl"].TrimEnd('/');
            await CreateApplicationAsync(
        ...
        //Web Client
        var webClientId = configurationSection["HieuEcommerce_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["HieuEcommerce_Web:RootUrl"].EnsureEndsWith('/');
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["HieuEcommerce_Web:ClientSecret"] ?? "1q2w3e*",
                grantTypes: new List<string> //Hybrid flow
                {
                    OpenIddictConstants.GrantTypes.AuthorizationCode,
                    OpenIddictConstants.GrantTypes.Implicit
